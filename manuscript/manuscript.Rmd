...
We found that differences in P retention among lake connectivity classes was influenced by specific connectivity metrics including average link length, closest lake distance, and stream density (Table 2, Figure \ref{fig7}). Indeed, these metrics were more strongly associated with P retention than covariates typically used in statistical P retention models (e.g. lake depth, Figure \ref{fig1}). Note that we were able to examine the influence of specific metrics as separate effects apart from water residence time because our model structure treats them as hierarchical coefficients on the P decay parameter $k$. As a result, although water residence time remains the dominant effect on lake P retention, we were able to estimate the specific influence of each metric in a way that is not possible using integrated water residence time and connectivity metrics such as watershed transport capacity \citep{fraterrigoInfluenceLandUse2008}.
