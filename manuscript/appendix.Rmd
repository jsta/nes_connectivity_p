...
ggplot() + 
  ...
  geom_sf(data = nws) +
  geom_sf(data = nws_lakes[1,], color = "cyan") +
  geom_sf(data = nws_lakes[2,], color = "yellow") +
...
```{r plot_tree_example, echo=FALSE, fig.width=2, fig.height=3}
plot_tree(gettree(readRDS("../data/nws/linklength_forest.rds")), 
          title = "Link Length")
```

```{r partition_example_scatter, fig.height=2, fig.width=2.2}
results_table <- read.csv("../scripts/table_1.csv", stringsAsFactors = FALSE)
ll_split <- dplyr::filter(results_table, 
                          nws_names == "avg_link_length" & 
                          scale == "nws")$splits
