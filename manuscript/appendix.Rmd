...
if(!file.exists(milstead_path)){
  ...
  milstead_j       <- dplyr::left_join(milstead, nhd_plus, by = c("WB_ID" = "COMID")) %>%
    dplyr::left_join(nhd, by = c("REACHCODE" = "ReachCode")) %>%
    dplyr::left_join(lg$iws.conn, by = c("Permanent_Identifier" = "iws_nhdid")) %>%
    dplyr::left_join(lg$locus, by = c("Permanent_Identifier" = "nhdid"))
...
milstead <- milstead[milstead$hrt > 0,] 
milstead <- milstead[milstead$Area > 0.04,] # rm less than 4 ha
...
quants <- exp(quantile(log(milstead[, "hrt"])))

gg_fit_single <- 
  ggplot(data = milstead, 
...
gg_fit_multi <- ggplot(data = milstead, aes(x = hrt, y = Rp)) +
  geom_point(size = 0.9) +
  scale_x_log10(labels = mixed_labels, 
                breaks = as.numeric(yr_labels), limits = c(1 / 365, 11)) +
  stat_smooth(method = "glm", method.args = list(family = "binomial"), 
...
if(!interactive()){  
  ggsave("milstead_multi.pdf", gg_fit_multi, height = 5)
}
