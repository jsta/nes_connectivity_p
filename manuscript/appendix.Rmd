...
```{r iws_vs_nws-gis, cache = TRUE, echo=FALSE}
# lagoslakeid <- 6302
...
ggplot() + 
  geom_sf(data = nws_outline, color = "cyan", size = 2) + 
  geom_sf(data = iws_outline, color = "cyan", size = 1, linetype = "dashed") + 
  geom_sf(data = nws) +
  # geom_sf(data = iws) + 
  theme_void()
...
```{r best_partition_maps, cache=TRUE, echo=FALSE}
# setwd("manuscript")
...
partition_splits <- dplyr::filter(partition_splits, d_k > 0.2)
...
plot_grid(plotlist = lower_nws_maps[2],
